...
<?php else : ?>
	...
	<?php foreach ($list as $language) : ?>
		<?php if (!$language->active) : ?>
			<li class="nav-item">
			<a class="nav-link" href="<?php echo htmlspecialchars_decode(htmlspecialchars($language->link, ENT_QUOTES, 'UTF-8'), ENT_NOQUOTES); ?>">
		...
		<?php elseif ($params->get('show_active', 1)) : ?>
			...
			<a class="nav-link" href="<?php echo htmlspecialchars_decode(htmlspecialchars($base, ENT_QUOTES, 'UTF-8'), ENT_NOQUOTES); ?>">
			<?php if ($params->get('image', 1)) : ?>
				<?php if ($language->image) : ?>
					<?php echo JHtml::_('image', 'mod_languages/' . $language->image . '.gif', $language->title_native, array('title' => $language->title_native), true); ?>
				<?php else : ?>
